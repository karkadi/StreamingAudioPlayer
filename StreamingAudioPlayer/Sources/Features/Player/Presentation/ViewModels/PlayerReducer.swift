...
struct PlayerReducer {
    ...
    var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case .startLiveActivity:
                return .run { [station = state.station, isPlaying = state.isPlaying] _ in
                    try await playerUseCase.startLiveActivity(station: station, isPlaying: isPlaying)
                }
                
            case .updateLiveActivity:
                return .run { [station = state.station, isPlaying = state.isPlaying] _ in
                    await playerUseCase.updateLiveActivity(station: station, isPlaying: isPlaying)
                }
                
            case .endLiveActivity:
                return .run { _ in
                    await playerUseCase.endLiveActivity()
                }
            }
        }
    }
