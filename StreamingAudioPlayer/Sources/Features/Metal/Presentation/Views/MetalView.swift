...
struct MetalView: UIViewRepresentable {
    let renderingUseCase: any MetalRenderingUseCase

    init?(fragmentFunction: String) {
        if let renderingUseCase = try? DefaultMetalRenderingUseCase(
            mtkView: MTKView(),
    ...
    func makeUIView(context: Context) -> MTKView {
        ...
        do {
            try renderingUseCase.configure(mtkView: mtkView)
        } catch {
    ...
    func makeCoordinator() -> Coordinator {
        Coordinator(renderingUseCase: renderingUseCase)
    }

    class Coordinator {
        private let renderingUseCase: any MetalRenderingUseCase

        init(renderingUseCase: any MetalRenderingUseCase) {
            self.renderingUseCase = renderingUseCase
        }
